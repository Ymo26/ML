#!/usr/bin/env python3
# CNN+LSTM Price Forecasting — KAN-like activation + Huber loss (fixed add_weight)
...
USE_BASELINES = True
HUBER_DELTA = 1.0
KAN_NUM_KER = 8   # number of Gaussian kernels in activation
# ====================================================
...
try:
    ...
    from tensorflow.keras import layers, models, callbacks, losses, constraints, initializers
except Exception:
...
class KANActivation1D(layers.Layer):
    """
    Trainable activation: y = x + sum_i a_i * exp(-b_i * (x - c_i)^2)
    Shared across time & channels for stability.
    """
    ...
    def get_config(self):
        cfg = super().get_config(); cfg.update({"num_kernels": self.num_kernels}); return cfg
...
def build_cnn_lstm_kan(input_shape, num_k=8):
    ...
    x=layers.LSTM(32)(x)
    x=layers.Dense(32, activation="linear")(x)  # nonlinearity provided by KAN layers
...
runs=[("CNN+LSTM(KAN)+Huber", lambda: build_cnn_lstm_kan(inp_shape, KAN_NUM_KER))]
if USE_BASELINES: runs += [("LSTM+Huber", lambda: build_lstm(inp_shape)), ("GRU+Huber", lambda: build_gru(inp_shape))]

OUT=Path("./out_kan_huber"); OUT.mkdir(parents=True, exist_ok=True)
...
def evaluate(m,name):
    ...
    plt.title(f"{name} (H={HORIZON})"); plt.legend(); plt.tight_layout()
    ...
    return {"Model":name,"Loss":"Huber","MAE":mae,"RMSE":rmse,"MAPE":mape,"R2":r2,"Directional_Acc":dira}
