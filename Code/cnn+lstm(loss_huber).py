#!/usr/bin/env python3
"""
CNN+LSTM Price Forecasting — Loss-Variant Edition (duplicate-dates safe)

- Same pipeline you have (duplicate-date fix, indicators, scaling, sliding windows).
- NEW: Pluggable loss (Huber/MAE/MSE/MAPE/Quantile-Pinball).

Run:
  source "/Users/luoyi/Desktop/ML Research/.venv/bin/activate"
  python cnn_lstm_forecasting_lossvariant.py
"""
...
# 🔁 Choose loss here: "huber", "mae", "mse", "mape", "pinball"
LOSS_NAME   = "huber"
HUBER_DELTA = 1.0        # for Huber
PINBALL_TAU = 0.5        # for Quantile (0.5 = median/MAE-like)
# -------------------------------------------------------------
...
def pinball_loss(tau: float):
    """Quantile (Pinball) loss for tau in (0,1). Robust & asymmetric."""
    tau = float(tau)
...
def get_loss(name: str):
    name = name.lower()
    if name == "huber":
        return losses.Huber(delta=HUBER_DELTA)
    if name == "mae":
    ...
    if name == "pinball":
        return pinball_loss(PINBALL_TAU)
    raise ValueError(f"Unknown loss: {name}")

# --------------------------- Helpers ---------------------------
def infer_date_price(df: pd.DataFrame) -> Tuple[str, str]:
    # honor explicit config if present
...
def add_tech_indicators(df: pd.DataFrame, price_col: str) -> pd.DataFrame:
    """Compact indicators: returns, log returns, MA/STD, RSI, MACD, Bollinger."""
...
# Parse/clean date, keep only date+price
...
# Collapse to one row per day (handles duplicate timestamps safely)
...
# Resample to daily calendar and fill gaps
...
# Scale on train portion only (no leakage)
...
# Use lambdas so we pass `loss_obj` at build time
...
# --------------------------- Train/Eval ---------------------------
OUT = Path(f"./outputs_cnnlstm_{LOSS_NAME}")
...
def evaluate(trained, name: str):
    ...
    plt.title(f"{name} — Actual vs Predicted (H={HORIZON}, loss={LOSS_NAME})")
