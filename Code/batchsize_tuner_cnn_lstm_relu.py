#!/usr/bin/env python3
# Batch-size tuner for CNN+LSTM (ReLU) + Huber, with LR √-scaling and OOM-safe sweep.
...
BATCH_GRID: List[int] = [16, 24, 32, 48, 64, 96, 128]   # safe for 8GB M2; add 192/256 if you can
EPOCHS_TUNE = 12
EPOCHS_FULL = 60

# Optimizer / loss (base LR tuned for ReLU @ REF_BS=64 from your runs)
BASE_LR     = 5e-4
REF_BS      = 64
HUBER_DELTA = 1.0
CLIPNORM    = 1.0
...
def sqrt_lr_scale(base_lr: float, batch_size: int, ref_bs: int = 64) -> float:
    """Adam-friendly √-scaling of LR with batch size."""
...
out_root = Path("./out_batch_tune_relu"); out_root.mkdir(parents=True, exist_ok=True)
rows = []

print(f"[INFO] Starting batch-size sweep (ReLU) over: {BATCH_GRID}")
...
print(f"\n[SELECT] ReLU best BS={BEST_BS} (scaled LR={BEST_LR:.6g})")

# ------------------ full train with best BS ------------------
m = build_cnn_lstm_relu(input_shape, BEST_LR)
cbs = [
...
pd.DataFrame({"date":idx.astype(str),"y_true":ytrue,"y_pred":yhat}).to_csv(out_root/f"pred_relu_bs{BEST_BS}.csv",index=False)
plt.figure(figsize=(10,4)); plt.plot(idx,ytrue,label="Actual"); plt.plot(idx,yhat,label="Pred")
plt.title(f"ReLU — Best BS={BEST_BS}, LR={BEST_LR:.6g}"); plt.legend(); plt.tight_layout()
plt.savefig(out_root/f"relu_best_bs{BEST_BS}_actual_vs_pred.png", dpi=120)
...
try: m.save(out_root/f"relu_best_bs{BEST_BS}.keras")
except: pass
