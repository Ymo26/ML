#!/usr/bin/env python3
# CNN+LSTM Price Forecasting — KAN-like activation in Conv blocks (trainable)
...
USE_BASELINES = True
LOSS_NAME   = "logcosh"
KAN_NUM_KER = 8      # number of Gaussian kernels in the activation
# ====================================================
...
# ---------- KAN-like activation (1D sequence) ----------
class KANActivation1D(layers.Layer):
    """
    Sum of Gaussians activation with residual:
        y = x + sum_i a_i * exp(-b_i * (x - c_i)^2)
    Shared parameters across channels/time (simple & stable).
    """
    ...
    def build(self, input_shape):
        self.amp   = self.add_weight("amp",   shape=(self.num_kernels,), initializer="zeros", trainable=True)
        self.cent  = self.add_weight("cent",  shape=(self.num_kernels,), initializer=initializers.RandomUniform(-1.0,1.0), trainable=True)
        self.beta  = self.add_weight("beta",  shape=(self.num_kernels,), initializer=initializers.Constant(1.0),
    ...
    def call(self, x):
        ...
        amp = tf.reshape(self.amp,  (1,1,1,self.num_kernels))
        cen = tf.reshape(self.cent, (1,1,1,self.num_kernels))
        bet = tf.reshape(self.beta, (1,1,1,self.num_kernels))
    ...
    def get_config(self):
        ...
        cfg.update({"num_kernels": self.num_kernels})
        return cfg
...
# ---------- models (KAN-like in Conv blocks) ----------
def build_cnn_lstm_kan(input_shape, num_k=8):
    ...
    x=layers.LSTM(32)(x)
    x=layers.Dense(32, activation="linear")(x)  # keep linear here; KAN is in conv path
...
runs=[("CNN+LSTM(KAN)", lambda: build_cnn_lstm_kan(inp_shape, KAN_NUM_KER))]
if USE_BASELINES: runs += [("LSTM", lambda: build_lstm(inp_shape)), ("GRU", lambda: build_gru(inp_shape))]

OUT=Path("./out_kan"); OUT.mkdir(parents=True, exist_ok=True)
