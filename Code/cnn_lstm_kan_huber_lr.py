#!/usr/bin/env python3
# CNN+LSTM — KAN-like activation + Huber loss + tuned learning rate (fixed add_weight)
...
HUBER_DELTA    = 1.0
LEARNING_RATE  = 7.5e-4  # slightly conservative for trainable KAN nonlinearity
MIN_LR         = 1e-6
PLATEAU_FACTOR = 0.5
PLATEAU_PATIENCE = 4
KAN_NUM_KER   = 8
...
try:
    ...
    from tensorflow.keras import layers, models, callbacks, losses, constraints, initializers, optimizers
except Exception:
...
class KANActivation1D(layers.Layer):
    """
    y = x + sum_i a_i * exp(-b_i * (x - c_i)^2)
    Shared across time & channels (stable, lightweight).
    """
    def __init__(self, num_kernels=8, **kwargs):
        super().__init__(**kwargs); self.num_kernels=int(num_kernels)

    def build(self, input_shape):
        self.amp  = self.add_weight(name="amp",  shape=(self.num_kernels,),
                                    initializer=initializers.Zeros(), trainable=True)
        self.cent = self.add_weight(name="cent", shape=(self.num_kernels,),
                                    initializer=initializers.RandomUniform(-1.0,1.0), trainable=True)
        self.beta = self.add_weight(name="beta", shape=(self.num_kernels,),
    ...
    def call(self, x):
        ...
        x_exp = tf.expand_dims(x, axis=-1)                 # (B,T,C,1)
        e = tf.exp(- bet * tf.square(x_exp - cen))         # (B,T,C,K)
        y = tf.reduce_sum(amp * e, axis=-1)                # (B,T,C)
        return x + y
...
def infer_date_price(df: pd.DataFrame) -> Tuple[str, str]:
    ...
    d=next((c for c in dcands if c in df.columns),None); p=next((c for c in pcands if c in df.columns),None)
...
loss_obj = losses.Huber(delta=HUBER_DELTA)
opt = optimizers.Adam(learning_rate=LEARNING_RATE, clipnorm=1.0)

def build_cnn_lstm_kan(input_shape, num_k=8):
    inp=layers.Input(shape=input_shape)
    x=layers.Conv1D(32,3,padding="causal",activation=None)(inp)
    x=KANActivation1D(num_kernels=num_k)(x)
    x=layers.Conv1D(32,3,padding="causal",activation=None)(x)
    x=KANActivation1D(num_kernels=num_k)(x)
    x=layers.MaxPooling1D(2)(x)
    x=layers.LSTM(64, return_sequences=True)(x)
    x=layers.LSTM(32)(x)
    x=layers.Dense(32, activation="linear")(x)  # nonlinearity from KAN layers
    out=layers.Dense(1)(x)
    m=models.Model(inp,out); m.compile(optimizer=opt, loss=loss_obj); return m

inp_shape=(X_tr.shape[1],X_tr.shape[2])
cb=[
    callbacks.EarlyStopping(patience=8,restore_best_weights=True,monitor="val_loss"),
    callbacks.ReduceLROnPlateau(monitor="val_loss", factor=PLATEAU_FACTOR, patience=PLATEAU_PATIENCE,
                                min_lr=MIN_LR, verbose=1),
]

OUT=Path("./out_kan_huber_lr"); OUT.mkdir(parents=True, exist_ok=True)
...
def evaluate(m,name):
    ...
    plt.title(f"{name} (H={HORIZON}, lr={LEARNING_RATE})"); plt.legend(); plt.tight_layout()
    ...
    return {"Model":name,"Loss":"Huber","LR":LEARNING_RATE,"MAE":mae,"RMSE":rmse,"MAPE":mape,"R2":r2,"Directional_Acc":dira}

name="CNN+LSTM(KAN)+Huber"
print(f"\n[TRAIN] {name} (lr={LEARNING_RATE}) ...")
m=build_cnn_lstm_kan(inp_shape, KAN_NUM_KER)
m.fit(X_tr,y_tr,validation_data=(X_va,y_va),epochs=EPOCHS,batch_size=BATCH,callbacks=cb,verbose=1)
res=evaluate(m,name); print(res)
try: m.save(OUT/f"{name.replace('+','_')}.keras")
except: pass
pd.DataFrame([res]).to_csv(OUT/"metrics_summary.csv", index=False)
