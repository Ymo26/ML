#!/usr/bin/env python3
# CNN+LSTM Price Forecasting — Sigmoid activations + Huber loss
...
runs=[("CNN+LSTM(Sigmoid)+Huber", lambda: build_cnn_lstm_sigmoid(inp_shape))]
if USE_BASELINES: runs += [("LSTM+Huber", lambda: build_lstm(inp_shape)), ("GRU+Huber", lambda: build_gru(inp_shape))]

OUT=Path("./out_sigmoid_huber"); OUT.mkdir(parents=True, exist_ok=True)
