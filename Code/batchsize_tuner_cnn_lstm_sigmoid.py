#!/usr/bin/env python3
# Batch-size tuner for CNN+LSTM (Sigmoid) + Huber, with LR √-scaling and OOM-safe sweep.
...
BATCH_GRID: List[int] = [16, 24, 32, 48, 64, 96, 128]   # safe for 8GB
EPOCHS_TUNE = 12
EPOCHS_FULL = 60

# Optimizer / loss
BASE_LR     = 8e-4   # tuned base LR for Sigmoid @ REF_BS=64 (from your plots)
REF_BS      = 64
HUBER_DELTA = 1.0
CLIPNORM    = 1.0
MIN_LR      = 1e-6
...
def build_cnn_lstm_sigmoid(input_shape, lr: float):
...
out_root = Path("./out_batch_tune_sigmoid"); out_root.mkdir(parents=True, exist_ok=True)
rows = []

print(f"[INFO] Starting batch-size sweep (Sigmoid) over: {BATCH_GRID}")
for bs in BATCH_GRID:
    lr = sqrt_lr_scale(BASE_LR, bs, REF_BS)
    print(f"\n[TUNE] BS={bs}, scaled LR={lr:.6g}")
    try:
        m = build_cnn_lstm_sigmoid(input_shape, lr)
...
print(f"\n[SELECT] Sigmoid best BS={BEST_BS} (scaled LR={BEST_LR:.6g})")

m = build_cnn_lstm_sigmoid(input_shape, BEST_LR)
cbs = [
...
pd.DataFrame({"date":idx.astype(str),"y_true":ytrue,"y_pred":yhat}).to_csv(out_root/f"pred_sigmoid_bs{BEST_BS}.csv",index=False)
plt.figure(figsize=(10,4)); plt.plot(idx,ytrue,label="Actual"); plt.plot(idx,yhat,label="Pred")
plt.title(f"Sigmoid — Best BS={BEST_BS}, LR={BEST_LR:.6g}"); plt.legend(); plt.tight_layout()
plt.savefig(out_root/f"sigmoid_best_bs{BEST_BS}_actual_vs_pred.png", dpi=120)
...
try: m.save(out_root/f"sigmoid_best_bs{BEST_BS}.keras")
except: pass
