#!/usr/bin/env python3
"""
CNN+LSTM Price Forecasting — Log-Cosh Loss Edition
- Duplicate-date safe loading (collapses duplicates to daily)
- Technical features (returns, MA/STD, RSI, MACD, Bollinger)
- Time-aware split, scaling, sliding windows (LOOKBACK, HORIZON)
- Models: CNN+LSTM (+ optional LSTM/GRU baselines)
- Loss: Log-Cosh (robust, smooth near 0, quadratic for small errors)

Run:
  source "/Users/luoyi/Desktop/ML Research/.venv/bin/activate"
  python cnn_lstm_forecasting_logcosh.py
"""
...
loss_obj = losses.LogCosh()  # <--- LOG-COSH here
...
# --------------------------- Train/Eval ---------------------------
OUT = Path("./outputs_cnnlstm_logcosh")
...
def evaluate(trained, name: str):
    ...
    plt.title(f"{name} — Actual vs Predicted (H={HORIZON}, loss=log-cosh)")
    ...
    return {"Model": name, "Loss": "log-cosh", "MAE": mae, "RMSE": rmse, "MAPE": mape, "R2": r2, "Directional_Acc": dira}

results = []
for name, builder in models_to_train:
    print(f"\n[TRAIN] {name} (loss=log-cosh) ...")
    m = builder()
