#!/usr/bin/env python3
# CNN+LSTM Price Forecasting — ReLU activations + Huber loss (duplicate-dates safe)
...
def add_indicators(df: pd.DataFrame, price_col: str) -> pd.DataFrame:
    ...
    ma20=s.rolling(20).mean(); sd20=s.rolling(20).std()
    out["bb_up"]=ma20+2*sd20; out["bb_dn"]=ma20-2*sd20
...
runs=[("CNN+LSTM(ReLU)+Huber", lambda: build_cnn_lstm_relu(inp_shape))]
if USE_BASELINES: runs += [("LSTM+Huber", lambda: build_lstm(inp_shape)), ("GRU+Huber", lambda: build_gru(inp_shape))]

OUT=Path("./out_relu_huber"); OUT.mkdir(parents=True, exist_ok=True)
